...
console.log('\n=== Reset por versión de llave (v3): las viejas se ignoran, arranca en 0 ===');
{
  const NUEVA = 'hitclaud.record.v3';
  const s = mockStorage();
  s.setItem('hitclaud.record', '99999');    // récord de una economía anterior
  s.setItem('hitclaud.record.v2', '55555'); // récord de la versión previa
  s.setItem('prefs.sonido', 'off');         // preferencia ajena (no se toca)
  // El juego arranca leyendo SOLO la llave nueva → récord 0.
  const r = U.crearRecord(s, NUEVA, 500);
  console.log(`  viejas .record=${s._data['hitclaud.record']} / .v2=${s._data['hitclaud.record.v2']} (ignoradas) → arranca en ${r.valor}  ${r.valor === 0 ? 'OK ✓' : 'NO ✗'}`);
  // Persiste en la llave NUEVA, sin tocar las viejas ni las preferencias.
  r.considerar(1234, 0); r.flush(0);
  console.log(`  persiste en '${NUEVA}'=${s._data[NUEVA]}  ${s._data[NUEVA] === '1234' ? 'OK ✓' : 'NO ✗'}`);
  const intactas = s._data['hitclaud.record'] === '99999' && s._data['hitclaud.record.v2'] === '55555' && s._data['prefs.sonido'] === 'off';
  console.log(`  viejas y preferencias intactas: ${intactas ? 'OK ✓' : 'NO ✗'}`);
}
